...
function generarFechaDemo(): Date {
  ...
  if (azar > 0.9) {
    ...
    return new Date(
      ahora.getTime() - Math.floor(Math.random() * 1.5 * 24 * 60 * 60 * 1000),
    );
  } else if (azar > 0.7) {
    // 20% son de este MES (Vista Mensual activa)
    return new Date(
      ahora.getTime() - Math.floor(Math.random() * 30 * 24 * 60 * 60 * 1000),
    );
  } else {
    // 70% son del resto del AÑO (Vista Anual activa y bien poblada)
    return new Date(
      ahora.getTime() - Math.floor(Math.random() * 365 * 24 * 60 * 60 * 1000),
    );
  }
}
...
function generarPacientes(): Paciente[] {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const fechaRegistro = new Date(
      now.getTime() - diasAntiguedad * 24 * 60 * 60 * 1000,
    );
...
function generarRecetas(
  ...
  const diagnosticos = [
    {
      ...
      med1: "Losartán",
      med2: "Hidroclorotiazida",
      ind: "Control diario de PA. Dieta baja en sodio.",
      esp: "cardiologia",
    },
    {
      dx: "Insuficiencia cardíaca congestiva",
      med1: "Enalapril",
      med2: "Furosemida",
      ind: "Restricción de líquidos. Vigilancia de edemas.",
      esp: "cardiologia",
    },
    {
      dx: "Arritmia cardíaca",
      med1: "Amiodarona",
      med2: "Aspirina",
      ind: "Toma de pulso diaria. Reposo relativo.",
      esp: "cardiologia",
    },
    {
      dx: "Infarto agudo al miocardio (Seguimiento)",
      med1: "Atorvastatina",
      med2: "Clopidogrel",
      ind: "Dieta cardioprotectora. No esfuerzos.",
      esp: "cardiologia",
    },
    {
      ...
      med1: "Metformina",
      med2: "Glibenclamida",
      ind: "Dieta baja en carbohidratos. Ejercicio regular.",
      esp: "general",
    },
    {
      dx: "Infección respiratoria aguda",
      med1: "Amoxicilina",
      med2: "Paracetamol",
      ind: "Abundantes líquidos. Reposo.",
      esp: "general",
    },
    {
      dx: "Gastritis aguda",
      med1: "Omeprazol",
      med2: "Butilhioscina",
      ind: "Evitar irritantes, café y alcohol.",
      esp: "general",
    },
    {
      ...
      med1: "Ceftriaxona",
      med2: "Ibuprofeno",
      ind: "Completar esquema de antibiótico.",
      esp: "general",
    },
  ];
  ...
  const buscarMedicamento = (
    nombre: string,
  ): (typeof catalogoMedicamentosInicial)[0] | undefined => {
    return catalogoMedicamentosInicial.find(
      (m) =>
  ...
  pacientes.forEach((paciente) => {
    ...
    for (let i = 0; i < numRecetas; i++) {
      ...
      const dxEspecializados = diagnosticos.filter(
        (d) => d.esp === especialidad,
      );
      const dxGenerales = diagnosticos.filter((d) => d.esp === "general");

      // 100% probabilidad de especialidad si existe y no es general
      const usarEspecialidad =
        especialidad !== "general" && dxEspecializados.length > 0;
      const dxData = getRandomItem(
        usarEspecialidad ? dxEspecializados : dxGenerales,
      );
  ...
  return recetas.sort(
    (a, b) => b.fechaEmision.getTime() - a.fechaEmision.getTime(),
  );
}
 ...
 */
function generarMovimientosFinancieros(
  recetas: Receta[],
): MovimientoFinanciero[] {
...
export async function seedDatabase(options: SeedOptions = {}): Promise<void> {
  const { isDemo = false, especialidad = "general", extraData } = options;

  try {
    console.log(
      `🌱 Iniciando población de base de datos (${isDemo ? "MODO DEMO" : "MODO REAL"})...\n`,
    );
    ...
    if (isDemo) {
      ...
      const doctorData = extraData?.doctor;
      ...
      const obtenerEtiquetaEspecialidad = (key: string): string => {
        ...
        const label = key
          .replace(/([A-Z])/g, " $1") // para saludMental -> Salud Mental
          .replace(/[_-]/g, " ")
          .trim();
        return label.charAt(0).toUpperCase() + label.slice(1);
      };

      const especialidadKeyFinal =
        doctorData?.especialidadKey ||
        (especialidad !== "general"
          ? especialidad
          : medicoBase.especialidadKey);
      ...
      const medicoFinal: MedicoConfig = {
        ...
        especialidad: String(
          doctorData?.especialidad ||
            obtenerEtiquetaEspecialidad(especialidadKeyFinal || ""),
        ),
      ...
      console.log(
        "👨‍⚕️ [DEMO] Guardando médico final en DB:",
        JSON.stringify(medicoFinal),
      );
      try {
      ...
      console.log(
        `✅ Médico: ${medicoFinal.nombre} (${medicoFinal.especialidadKey})\n`,
      );
      ...
      console.log(
        `📋 [DEMO] Insertando recetas para especialidad: ${especialidad}...`,
      );
      ...
      console.log(
        `✅ ${movimientos.length} movimientos financieros insertados\n`,
      );
    }
    ...
    const { catalogSyncService } =
      await import("@/shared/services/catalog-sync.service");
