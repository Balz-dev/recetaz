/**
 * @fileoverview Script de Población de Base de Datos
 * 
 * Este script genera datos de ejemplo para la base de datos IndexedDB del sistema
 * de recetas médicas. Incluye:
 * - Configuración del médico
 * - Pacientes de ejemplo
 * - Recetas médicas con medicamentos
...
// Polyfill para IndexedDB en Node.js
import 'fake-indexeddb/auto';

import Dexie from 'dexie';
import { v4 as uuidv4 } from 'uuid';

// Definición de tipos (copiados de src/types/index.ts)
interface Paciente {
    id: string;
    nombre: string;
    edad?: number;
...
class RecetasDatabase extends Dexie {
    medico!: Dexie.Table<MedicoConfig>;
    pacientes!: Dexie.Table<Paciente>;
    recetas!: Dexie.Table<Receta>;
    finanzas!: Dexie.Table<MovimientoFinanciero>;
    configuracionFinanciera!: Dexie.Table<ConfiguracionFinanciera>;
...
async function seed() {
    try {
        ...
        console.log('💡 Puedes iniciar la aplicación con: pnpm run start');
